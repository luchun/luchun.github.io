...
(function($){
  ...
  Showselect.prototype = {
    ...
    render:function(){
      ...
      context.dateArray.map(function(text,index){
        var item = $('<div  class="select-section-item" data-index="'+ index +'">'+ text +'<span class="check"></span></div>');
  ...
  function infoSelect(dateArray,multi){
    ...
    data.map(function(v){selected.push(dateArray[v]);});
    $this.siblings('input').val(selected.join(' '));
